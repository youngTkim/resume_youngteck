...
export class Text {
  ...
  setText(str, density, stageWidth, stageHeight) {
    console.log(`setText ${str} ${density} ${stageWidth}, ${stageHeight}`);
    // 텍스트가 배치될 canvas의 너비와 높이를 설정
    this.canvas.width = stageWidth;
    this.canvas.height = stageHeight;

    // 텍스트 내용과 폰트 크기와 종류 설정
    const myText = str;
    const fontWidth = 100;
    const fontSize = 240;
    ...
    // fillText할 때에 x,y 좌표값에 텍스트의 중앙이 위치
  ...
  dotPos(density, stageWidth, stageHeight) {
    ...
    //생성된 텍스트 이미지의 이미지를 이루는 이미지 데이터의 파티클들을 particles 배열로 return
